...
// InitEyeGazeCSVDir 供 main.go 调用，初始化眼动 CSV 文件目录。
...
// UploadEyeGazeDataCSV 在任务结束时，把 openxr_eye_gaze.csv 上传给甲方 A。
// 会使用 CurrentTask 中的 taskId 和 userNumber 作为 query 参数。
func UploadEyeGazeDataCSV() error {
	if CurrentTask == nil {
	...
	if eyeGazeCSVDir == "" {
		return fmt.Errorf("眼动CSV目录未初始化（eyeGazeCsvDir 为空）")
	}
	...
	part, err := writer.CreateFormFile("file", filepath.Base(csvPath))
	if err != nil {
		return fmt.Errorf("创建眼动表单文件字段失败: %w", err)
	}
...
func ReceiveStopHandler(w http.ResponseWriter, r *http.Request) {
	...
	if payload.Type == 4 {
		...
		if err := UploadEyeGazeDataCSV(); err != nil {
