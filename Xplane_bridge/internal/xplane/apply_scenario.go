...
// SetZuluTimeSec 单独设置 X-Plane 的 Zulu time（秒，0~86399）。
// 这是“环境初始化”的一部分：不需要开启 override_planepath。
func (c *Client) SetZuluTimeSec(ctx context.Context, zuluSec int64) error {
	if zuluSec < 0 {
		return fmt.Errorf("zuluSec 不能为负数: %d", zuluSec)
	}
	if zuluSec > 86399 {
		return fmt.Errorf("zuluSec 超出范围(0~86399): %d", zuluSec)
	}
	if err := c.initScenarioDatarefs(ctx); err != nil {
		return err
	}
	if err := c.SetDatarefValue(ctx, scDr.zuluTimeID, zuluSec); err != nil {
		return fmt.Errorf("写 zulu_time_sec 失败: %w", err)
	}
	return nil
}
